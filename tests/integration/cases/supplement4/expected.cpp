...
// 1.1 基础二元运算符（含多余空格）
int  operator+(const MyClass &a,const MyClass &b) {return a.x + b.x;}
...
void test_lambdas() {
    ...
    (void)l6;(void)l7;(void)l8;(void)l9;(void)l10;
}
...
int main() {
    ...
    a = b;
    bool ok = (a == b);
    int  sum = operator+(a,b);
    (void)ok;
    (void)sum;

    test_lambdas();
    test_if_chain(1);
    test_tight_else_if(1);
    test_nested_if(1,0);
    test_control_keywords(1);

    ConstructorTest ct(1,2);
    ConstructorTest ct2;
    ConstructorTest ct3(3.0);
    (void)ct.get_x();
    (void)ct2.get_y();
    (void)ct3.get_z();
